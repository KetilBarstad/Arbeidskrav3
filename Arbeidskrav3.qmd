...
ggplot(sample.powerlifting, aes(x = Age, y = TotalKg, color = Sex)) +
  ...
  labs(
    title = "Graf 2: Polynomialregresjon",
    x = "Alder",
    y = "Total (Kg)"
  ) +
  theme_minimal()
...
predict(ext.m2, newdata = data.frame(legpress = 150,
                                      sex = "female")) 
predict(ext.m2, newdata = data.frame(legpress = 150,
                                     sex = "male"))
...
predict(ext.m3, newdata = data.frame(legpress = 150,
                                     sex = "female"))
predict(ext.m3, newdata = data.frame(legpress = 150,
                                     sex = "male"))
...
predict(ext.m1, newdata = data.frame(legpress = 150))
...
Prediksjoner <- data.frame(
  ...
  Prediction = c(
    predict(ext.m1, newdata = data.frame(legpress = 150)),
    predict(ext.m2, newdata = data.frame(legpress = 150, sex = "female")),
    predict(ext.m2, newdata = data.frame(legpress = 150, sex = "male")),
    predict(ext.m3, newdata = data.frame(legpress = 150, sex = "female")),
    predict(ext.m3, newdata = data.frame(legpress = 150, sex = "male"))
  )
)

knitr::kable(Prediksjoner, digits = 2, caption = "Tabell 1: Prediksjoner fra de ulike modellene")
...
knitr::kable(pred_intervals, digits = 2, 
             caption = "Tabell 2: 95% prediksjonsintervall for legextension ved legpress = 150 kg")
```
...
Hvis en kvinne tar 150kg i legpress, forventer vi at nye kvinner har en 95% sannsynlighet for å få en legextension verdi mellom 27 og 75kg. Modellen viser oss en gjennomsnittlig prediksjon på 51kg. Hvis en mann tar 150 kg i legpress, forventer vi nye menn har en 95% sannsynlighet for å få en legextension verdi mellom 43 og 91 kg. Modellen viser oss en gjennomsnittlig prediksjon på 68 kg.
...
![Tabell 3: Mixed effect modellen](Figurer/Mixed_effect.png)

![Graf 3: Linjediagram av mixed-effect modellen. Endring i load fra pre- til post](Figurer/Graf.png){width=50%}
